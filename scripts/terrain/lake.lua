...
--thelimit.terrain.void_lake = function(context)
...
--				for y = 5, -5, -1 do
--					pos.y = y
--					node = context.get_node(pos)
--					if node == HYPHUM_ID and has_walls(context.get_node) then
--						context.set_node(pos, VOID_FLUID_SOURCE_ID)
--						goto iter_break
--					end
--				end
--				::iter_break::
...
--thelimit.terrain.void_lake = function(context)
--	local radius = math.random(math.floor(context.max_side * 0.25), math.floor(context.max_side * 0.25))
...
--						else
--							if y == 0 then
--								context.set_node(pos, HYPHUM_ID)
--							else
--								context.set_node(pos, VITILIT_ID)
--							end
--						end
...
thelimit.terrain.void_lake = function(context)
	local radius = math.floor(context.max_side * 0.75)
	for z = -radius, radius do
		pos.z = z
		for x = -radius, radius do
			pos.x = x
			pos.y = -3
			local node = context.get_node(pos)
			if node ~= minetest.CONTENT_AIR then
